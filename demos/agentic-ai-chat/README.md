...
| An exported chat downloads as `chat-export.md` instead of a title-derived filename | Expected once a chat's title has no character safe for a filename (`docs/06-AGENTIC-CHAT.md` Phase 12) -- the document itself is unaffected; only the suggested filename falls back. |
| `GET /api/chats/:id/export`/`.../files/:fileId/export` returns `404` | Confirm you are signed in as the chat's owner; for the per-file route, also confirm the file id belongs to the same chat id in the URL -- both are checked, and every mismatch (including a foreign identity) reports the same `404` as a nonexistent chat/file (docs/06-AGENTIC-CHAT.md Phase 12, mirroring Phase 9's own file-download route). |
