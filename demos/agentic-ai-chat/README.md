...
- D1 database `<DEMO_NAME>-db`, bound as `DB`, holding the `users` table (its `business`/`geo` columns, Phase 7, are nullable application-level enums with no `CHECK` constraint), `chats`, `chat_usage` (this demo's per-turn cost ledger, Phase 6), and `chat_files` (agent-generated file metadata, Phase 9).
...
- R2 bucket `<DEMO_NAME>-files`, bound to the Worker as `FILES` (Phase 9) -- the only writer is `ChatAgent`'s own `writeMarkdown` tool.
