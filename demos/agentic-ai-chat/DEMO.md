...
40. In any chat, type "Please fetch https://developers.cloudflare.com/workers/ and tell me what it's about in one sentence." Point out the response streams in as normal and actually describes real page content — the agent fetched a real URL, not just answered from its own training data.
41. In the same chat, type "Please fetch https://cloudflare.com/." Point out the assistant's reply explains that destination is not allowed, rather than the turn failing, hanging, or crashing.
42. Switch to the Workers Logs tab (or open **Workers & Pages** > `agentic-chat` > **Logs** now) and find the two `egress_gateway_decision` entries from steps 40/41 — point out one shows `"allowed":true` for `developers.cloudflare.com` and the other `"allowed":false` for `cloudflare.com`, both carrying this chat's own id, and that the block happened at the platform layer (the `EgressGateway`'s own allow-list), not merely in the tool's application code.
43. Still signed in as the administrator, click **Skills** in the header. Point out the empty "Your skills" list, then add a personal skill: name it `spike-fact`, describe it as "Use whenever the user asks for the demo passphrase," choose "Paste content," and enter "The demo passphrase is TURQUOISE-NARWHAL-77." as its instructions. Point out it appears in the list immediately.
44. Open (or start) any chat and ask "What is the demo passphrase?" Point out:
    - The response includes the exact passphrase from step 43 — a fact the model has no other way to know.
    - A chip labeled `spike-fact` appears in the transcript, distinct from an attachment chip, showing the skill activated for this turn.
45. Ask an unrelated question in the same chat (for example "What is 12 times 7?"). Point out the reply answers directly with no skill chip — the skill's own catalog entry (its name/description only) costs nothing in the prompt until it actually matches.
46. Switch to the R2 tab and open the `agentic-chat-files` bucket. Point out a `skills/personal/<your-email>/<skill-id>/SKILL.md` object — the skill's real content is stored the same way an agent-generated file is, disjoint from the `chats/` prefix.
47. Back in the browser, delete the `spike-fact` skill from the Skills page. Ask the same question from step 44 again (in a new chat, or wait about a minute for the cached catalog to refresh) and point out the agent no longer knows the passphrase — removing a skill removes its effect.
48. In the Admin Console's "Enterprise skills" section, add the same skill again the same way. Sign out and sign back in as the non-administrator identity from the Demonstration Prerequisites, open a new chat, and ask the same question. Point out this identity — who never added anything themselves — still gets the passphrase and the same activation chip: an enterprise skill is visible to every identity, not only the administrator who added it.
49. Sign back in as the administrator and delete the enterprise skill from the Admin Console.
50. Open the chat from step 34 (the one with the packing-list file attached). Click **Export chat** in the chat header. Point out the browser downloads a single Markdown file — open it in a text editor and show it contains every turn from this chat in order (including the `writeMarkdown` tool call and its result), and a "Cost And Token Summary" section near the top with this chat's own total cost and a confirmation-ratio line ("N of M turns confirmed by AI Gateway") matching the same figures the chat header's own cost badge already showed.
51. In the same chat's transcript, click the small download icon next to the packing-list attachment chip (distinct from the chip itself, which downloads the raw file). Point out this downloads a **different** Markdown file — open it and show it wraps the exact same packing-list content below a "Cost Context Of The Producing Turn" section naming the model, cost, and prompt/completion token counts for the specific turn that produced this file, not the whole chat's total.
52. Open a private/incognito window and sign in as the non-administrator identity. Attempt to open the export URL from step 50 directly (copy it from the first browser's network tab or address bar). Point out it returns an error, not the document — exports are ownership-checked exactly like the file-download route from step 38.
...
- Asking the agent to fetch an allow-listed URL yields a reply describing that page's real content; asking it to fetch a non-allow-listed URL yields a clear refusal explanation instead of a failed or hanging turn — both enforced by `EgressGateway`'s own allow-list, logged for every decision.
- A personal skill's presence changes the agent's behavior only for a turn that actually matches it, visibly (a distinct transcript chip), and is invisible to every other identity; an enterprise skill has the same effect but is visible to everyone. Deleting either removes its effect on the next matching question.
- Exporting a chat downloads a single Markdown document covering every turn, tool call/result, and skill activation, plus a cost/token summary matching the chat header's own figures; exporting a generated file downloads a document wrapping that file's own content with the cost/token context of the specific turn that produced it. Both are rejected for a non-owner exactly like the underlying file-download route.
