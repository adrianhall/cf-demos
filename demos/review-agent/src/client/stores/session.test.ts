...
describe("useSessionStore", () => {
  ...
  it("stores the verified identity returned by GET /api/me", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ email: "reviewer@example.com" }), {
          status: 200,
        }),
      ),
    );
    ...
    expect(store.email).toBe("reviewer@example.com");
    expect(store.isAuthenticated).toBe(true);
    expect(store.loading).toBe(false);
    expect(store.error).toBeNull();
  });

  it("clears the identity and exposes the server's own problem-detail message on failure", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
