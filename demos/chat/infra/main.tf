...
# The Durable Object namespace that backs ChatRoom (one instance per channel) is created and torn
# down with the Worker itself via `wrangler deploy`'s `durable_objects`/`migrations` config in
# `wrangler.jsonc.tpl` — it needs no separate Terraform resource. See AGENTS.md (Resource
# Ownership) and docs/04-ENTERPRISE-CHAT.md (Phase 1).
