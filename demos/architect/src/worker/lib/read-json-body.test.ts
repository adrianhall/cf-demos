...
/** Build a `POST` request with the given raw body text, matching `demos/url-shortener`'s test. */
function postRequest(body: string): Request {
  return new Request("https://example.com/api/diagrams", {
    body,
    method: "POST",
  });
}
...
describe("readJsonBody", () => {
  it("parses a well-formed JSON object body", async () => {
    await expect(
      readJsonBody(postRequest(JSON.stringify({ title: "My Diagram" }))),
    ).resolves.toEqual({ title: "My Diagram" });
  });

  it("parses any valid JSON value, not only objects", async () => {
    // readJsonBody only parses JSON; shape validation is the caller's validation module's job.
