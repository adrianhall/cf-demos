resource "cloudflare_worker" "demo" {
  ...
  # Workers AI is an account capability reached through the `AI` binding declared in
  # wrangler.jsonc.tpl, not a provisioned resource — there is nothing here for Terraform to
  # create, and therefore no `depends_on` entry for it (contrast with a D1/KV/R2/Queue binding,
  # which would require one — see AGENTS.md, Resource Ownership). Teardown has no corresponding
  # product-specific cleanup step for the same reason.
}
