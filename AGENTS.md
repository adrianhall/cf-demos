...
- Commit a Wrangler configuration template (`wrangler.jsonc.tpl`) for
  production generation, and a separate, static, committed `wrangler.local.jsonc`
  with hardcoded local placeholder binding identifiers for every local and CI
  command (`dev`, `build`, `typecheck`, integration tests). `wrangler.local.jsonc`
  requires no generation step and no Terraform state, so formatting, type
  checking, tests, and local builds work from a clean checkout. Never generate
  or overwrite `wrangler.local.jsonc`; it is source, not build output. The
  gitignored `wrangler.jsonc` is regenerated by every `npm run deploy` from
  Terraform outputs and is never used for local commands.
- Point the Cloudflare Vite plugin, `@cloudflare/vitest-pool-workers`, and any
  other local-only tool explicitly at `wrangler.local.jsonc` (for example via
  the `CLOUDFLARE_VITE_WRANGLER_CONFIG_PATH` environment variable or a
  `configPath` option) rather than relying on default config-file discovery,
  so a stale generated `wrangler.jsonc` from a prior deploy can never be picked
  up silently by a local command.
